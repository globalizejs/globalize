...
QUnit.test( "should parse era (G|GG|GGG|GGGGG)", function( assert ) {
	date1 = new Gdate.calendars.hebrew( 0, 4, 1, 1 );
	assertParse( assert, "AM 4", "G y", cldr, date1 );
	assertParse( assert, "AM 4", "GG y", cldr, date1 );
	assertParse( assert, "AM 4", "GGG y", cldr, date1 );
	assertParse( assert, "AM 4", "GGGG y", cldr, date1 );
	assertParse( assert, "AM 4", "GGGGG y", cldr, date1 );
});
...
QUnit.test( "should parse month (M|L) with no padding", function( assert ) {
	date1 = new Gdate.calendars.hebrew( 0, 5776, 1, 1 );
	assertParse( assert, "1 5776", "M y", cldr, date1 );
	assertParse( assert, "1 5776", "L y", cldr, date1 );
	date2 = new Gdate.calendars.hebrew( 0, 5776, "7-leap", 1 );
...
QUnit.test( "should parse day (d) with no padding", function( assert ) {
	date1 = new Gdate.calendars.hebrew( 0, 5776, 1, 2 );
	assertParse( assert, "2 1 5776", "d M y", cldr, date1 );

	date2 = new Gdate.calendars.hebrew( 0, 5776, "7-leap", 29 );
	assertParse( assert, "30 7 5776", "d M y", cldr, null );
	assertParse( assert, "29 7 5776", "d M y", cldr, date2 );

	date2 = new Gdate.calendars.hebrew( 0, 5775, 7, 29 );
