...
class Globalize3Test < MiniTest::Spec
  ...
  it "create uses the given locale" do
  ...
  it "attributes= uses the given locale" do
  ...
  it "saves a translations record for each locale using a given locale" do
    post = Post.create(:title => 'Titel', :locale => :de)
  ...
  it "saves a translations record for each locale using the current I18n locale" do
  ...
  it "reload works with translated attributes" do
  ...
  it "reload works with translated attributes when updated elsewhere" do
    post = Post.create(:title => 'foo')
  ...
  it "reload accepts standard finder options" do
    post = Post.create(:title => "title")
  ...
  it "to_xml includes translated fields" do
  ...
  it "to_xml doesn't affect untranslated models" do
    blog = Blog.create(:description => "my blog")
    blog.reload
    assert blog.to_xml =~ %r(<description>my blog</description>)
  end

  it "translated_locales returns locales that have translations" do
  ...
  it "a model with an after_save callback that reloads the model still saves correctly" do
  ...
  it "with_translations eager loads translations" do
  ...
  it "a subclass of an untranslated model can translate attributes" do
    post = Post.create(:title => 'title')
  ...
  it "modifiying translated attributes on a subclass of an untranslated model works" do
    post = Post.create(:title => 'title')
    translated_comment = TranslatedComment.create(:post => post, :content => 'content')
  ...
  it "calling translates a second times adds the new attributes to the translated attributes" do
  ...
  it "read_attribute for unsaved translation" do
  ...
  it "read_attribute for unsaved untranslated model with locale column" do
