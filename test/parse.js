...
test("format-parse all, float", function() {
	$.each(Globalize.cultures, function(culture, cultureInfo) {
		...
		var formatted = Globalize.format(value, "n", culture);
		equal( Globalize.parseFloat(formatted, 10, culture), value, culture + ": " + formatted );
	});
});
...
test("format-parse all, currency", function() {
	$.each(Globalize.cultures, function(culture, cultureInfo) {
		...
		var formatted = Globalize.format(value, "c", culture);
		equal( Globalize.parseFloat(formatted, culture), value, culture + ": " + formatted );
	});
});
...
test("format-parse all, date", function() {
	...
	$.each(Globalize.cultures, function(culture, cultureInfo) {
		...
		value.setTime(0);
		var formatted = Globalize.format(value, "F", culture);
		deepEqual( Globalize.parseDate(formatted, "F", culture), value, culture + ": " + formatted );
	});
});
